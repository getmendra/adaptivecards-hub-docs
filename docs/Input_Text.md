---
title: "Input.Text"
source: "https://adaptivecards.microsoft.com/?topic=Input.Text"
topic: "Input.Text"
---
...
| --- | --- | --- | --- | --- |
| type | string | Must be Input.Text. | 1.0 |
...
| inlineAction | object | The action that should be displayed as a button alongside the input. Action.ShowCard is not supported. Valid values: \`Action.Execute\`, \`Action.InsertImage\`, \`Action.OpenUrl\`, \`Action.OpenUrlDialog\`, \`Action.Popover\`, \`Action.ResetInputs\`, \`Action.RunCommands\`, \`Action.Submit\`, \`Action.ToggleVisibility\` | 1.0 |
| isMultiline | boolean | false | Controls if the input should allow multiple lines of text. | 1.0 |
...
| maxLength | number | The maximum length of the text in the input. | 1.0 |
| placeholder | string | The text to display as a placeholder when the user hasn't entered a value. | 1.0 |
| regex | string | The regular expression to validate the input. | 1.3 |
...
| style | string | "Text" | The style of the input. Valid values: \`Text\`, \`Tel\`, \`Url\`, \`Email\`, \`Password\` | 1.0 |
